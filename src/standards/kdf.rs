...
impl std::fmt::Display for KdfError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            KdfError::HmacCreationFailed => write!(f, "Failed to create HMAC for key derivation"),
        }
    }
}
...
impl From<crate::error::NexError> for KdfError {
    fn from(_err: crate::error::NexError) -> Self {
        KdfError::HmacCreationFailed
    }
}
...
pub fn derivate_key(password: &[u8], salt: &[u8], iterations: u32, length: usize) -> Result<Vec<u8>, KdfError> {
    let mut derived_key = Vec::with_capacity(length);
    let hmac = HmacNex::new(password).map_err(|_| KdfError::HmacCreationFailed)?;
